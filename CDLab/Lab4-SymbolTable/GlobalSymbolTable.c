...
const char *dataTypes[] = {"bool", "char", "double", "float", "int", "long", "short"};
const int dataTypesSizes[] = {1, 1, 8, 4, 4, 8, 2};
const int dataTypesLen = 7;
...
void insertSymbolNode(SymbolNode * prev, char * name, const char * type, int size) {
...
void main() {
    ...
    list->next = NULL;
    root = list;
    SymbolNode * symbolList = (SymbolNode *) malloc(sizeof(SymbolNode)), *symbolListRoot;
	...
	if (fptr1 != NULL) {
						...
						if (i > 0) {
							...
							if (!type) {
                                if (dataTypeInd != -1 && (!symbolExists(symbolListRoot, word))) {
                                    insertSymbolNode(symbolList, word, dataTypes[dataTypeInd], dataTypesSizes[dataTypeInd]);
        ...
        //     root = root -> next;
        // }
        printf("\nSYMBOL TABLE:\nNo.\tName\tDataType\tSize\n");
        int i = 1;
        symbolListRoot = symbolListRoot -> next;
        while (symbolListRoot != NULL) {
            printf("%d\t%s\t%s\t\t%d\n", i++, symbolListRoot->symbol.name, symbolListRoot->symbol.type, symbolListRoot->symbol.size);
            symbolListRoot = symbolListRoot -> next;
        }
	}
	else
